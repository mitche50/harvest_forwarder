...
describe("HarvestForwarder", function () {
	...
	describe("Admin functions", function () {
		...
		it("Should return a new owner once it's changed", async function () {
			expect(await forwarder.owner()).to.equal(owner.address);

			const setOwner = await forwarder.set_owner(
				"0x1a2cDCD5D6363c89420D926B3720C07f3b10a012"
			);

			// wait until the transaction is mined
			await setOwner.wait();

			expect(await forwarder.owner()).to.equal(
				"0x1a2cDCD5D6363c89420D926B3720C07f3b10a012"
			);
		});

		it("Should not allow a non-owner address to perform permissioned functions", async function () {
			...
			await expect(
				forwarder
					.connect(badActor)
					.set_tree("0x0000000000000000000000000000000000000000")
			).to.be.revertedWith("!owner");

			await expect(
				forwarder
					.connect(badActor)
					.set_owner("0x0000000000000000000000000000000000000000")
			).to.be.revertedWith("!owner");
		});
	});
